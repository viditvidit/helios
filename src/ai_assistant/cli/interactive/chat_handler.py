...
class ChatHandler:
    ...
    async def _handle_file_apply_logic(self, response_content: str):
        ...
        # This part of the logic will be triggered by the /apply command
        # or can be integrated here with a prompt. For now, we just notify.
        # The full diff-and-apply logic is in `actions_impl.py` `handle_apply_changes`.
    ...
    async def _stream_and_render_response(self, request: CodeRequest):
        ...
        try:
            ...
            with console.status("[cyan]Helios is thinking...[/cyan]", spinner="point"):
            ...
            if not file_blocks:
                ...
                for block in file_blocks:
                    ...
                    # Render the file content as Markdown inside its own panel.
                    md_content = Markdown(block['code'], code_theme="vim")
                    console.print(Panel(
                        md_content,
                        title=f"File: {block['filename']}",
                        border_style="blue",
                        expand=False
                    ))
            ...
            # This is expected on Ctrl+C, so just print a newline to be clean.
            console.print()
    ...
    async def handle(self, message: str, session):
        ...
        try:
            ...
            if found_paths:
                console.print("[dim]Processing file and image mentions...[/dim]")
                for mention in set(found_paths):
                    ...
                    if mimetype and mimetype.startswith('image/'):
                        if not self.config.get_current_model().multimodal:
                            console.print(f"[bold red]Model '{self.config.model_name}' does not support image inputs.[/bold red]")
                            return
                        console.print(f" [dim]Adding image context from: {mention}[/dim]")
                        try:
                            with open(full_path, "rb") as image_file:
                                encoded_string = base64.b64encode(image_file.read()).decode('utf-8')
                                image_data_b64.append(encoded_string)
                        except Exception as e:
                            console.print(f"[red]Error reading image file {mention}: {e}[/red]")
                        continue
