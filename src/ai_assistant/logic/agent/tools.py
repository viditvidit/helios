...
async def review_and_commit_changes(session, commit_message: str, show_diff: bool = True) -> bool:
    ...
    with console.status(f"[cyan]Checking for changes...[/cyan]"):
...
async def run_shell_command(session, command: str, cwd: str, can_fail: bool = False, verbose: bool = False) -> bool:
    """Executes a shell command with AI-powered self-correction."""
    run_dir = Path(cwd)
    console.print(f"Running command: [bold magenta]{command}[/bold magenta] in [dim]{run_dir}[/dim]")

    try:
        with console.status(f"[cyan]Executing command...[/cyan]"):
        ...
        if process.returncode == 0:
            console.print("[green]✓ Shell command finished successfully.[/green]")
            if verbose and stdout_text:
                console.print(Panel(stdout_text, title="Output", border_style="dim"))
            return True
        
        console.print("[bold red]Shell command failed. Attempting self-correction...[/bold red]")
        ...
        with console.status("[cyan]Asking AI for a fix...[/cyan]"):
            corrected_command = await _get_ai_corrected_command(session, command, stderr_text)
        ...
        console.print(f"AI suggests this correction: [bold yellow]{corrected_command}[/bold yellow]")
